  ...
  async execute(interaction){
    if (interaction.isStringSelectMenu()) {
        if(interaction.customId == 'select_bot') {
          ...
          if(selected == "BuyNormalBroadcast") {
          ...
          let normalBroadcastPrice = parseInt(prices.get(`normalBroadcast_price_${interaction.guild.id}`))
          if(!normalBroadcastPrice) normalBroadcastPrice = 20;
...
userbalance = parseInt(usersdata.get(`balance_${interaction.user.id}_${interaction.guild.id}`))
        if(userbalance < normalBroadcastPrice) return interaction.reply({content:`**انت لا تمتلك الرصيد الكافي**` , ephemeral:true})
           const modal = new ModalBuilder()
          .setCustomId('BuyNormalBroadcast_Modal')
       .setTitle('Make Normal Broadcast Bot');
