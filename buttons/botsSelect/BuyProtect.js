  ...
  async execute(interaction){
    if (interaction.isStringSelectMenu()) {
        if(interaction.customId == 'select_bot') {
          ...
          let protectPrice = parseInt(prices.get(`protect_price_${interaction.guild.id}`))
          if(!protectPrice) protectPrice = 20;
...
userbalance = parseInt(usersdata.get(`balance_${interaction.user.id}_${interaction.guild.id}`))
        if(userbalance < protectPrice) return interaction.reply({content:`**انت لا تمتلك الرصيد الكافي**` , ephemeral:true})
           const modal = new ModalBuilder()
          .setCustomId('BuyProtect_Modal')
       .setTitle('Make protect Bot');
